...
interface ValueEstimatorResultsProps {
  userId?: string;
}

interface PropertyEstimate {
  property_estimate_id: string;
  owner_id: string;
  id: string;
...
export default function ValueEstimatorResults({ userId }: ValueEstimatorResultsProps) {
  ...
  console.log('🏠 ValueEstimatorResults component mounted, estimateId:', estimateId, 'userId:', userId);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Fetch the main estimate - filter by BOTH property_estimate_id AND owner_id for RLS
        const { data: estimateData, error: estimateError } = await supabase
          .from('property_estimates')
          .select('*')
          .eq('property_estimate_id', estimateId)
          .eq('owner_id', userId)
          .single();
  ...
  useEffect(() => {
    ...
    const initMap = () => {
      const mapDiv = document.getElementById('value-estimate-map');
      ...
      comps.forEach((comp) => {
        new google.maps.Marker({
          ...
          title: `${comp.formattedAddress} - $${comp.price?.toLocaleString()}`,
        });
      });
      ...
      <div className="p-6">
        <div className="max-w-6xl mx-auto">
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 dark:text-gray-400">Loading your property value estimate...</p>
          </div>
        </div>
      </div>
    );
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
          ...
          <div className="lg:col-span-2 space-y-6">
                  ...
                  <>
                    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border-4 border-brand-500 p-6">
                      ...
                      <p className="text-3xl font-bold text-gray-900 dark:text-gray-100">
                        ${estimate.price?.toLocaleString() || '0'}
                      </p>
                    </div>
                    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                      <p className="text-sm text-gray-600 dark:text-gray-400 mb-1">PRICE RANGE</p>
                      <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">
                        ${estimate.priceRangeLow?.toLocaleString() || '0'} - ${estimate.priceRangeHigh?.toLocaleString() || '0'}
                      </p>
                    </div>
                    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
              <div
                id="value-estimate-map"
                className="w-full h-96 rounded-lg bg-gray-100 dark:bg-gray-700"
              />
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-2 text-center">
                Red marker: Subject property • Blue markers: Comparable properties
              </p>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-brand-500 to-brand-600 rounded-lg shadow-lg p-6 text-white">
              ...
              <p className="text-sm text-brand-50 mb-4">
                Download a detailed Comparative Market Analysis (CMA) report for {estimate.formattedAddress} using the latest available market data.
              </p>
              <button
